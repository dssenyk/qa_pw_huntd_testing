...
>({
  ...
  recruiterUser: async ({ factories }, use) => {
    const recruiterUser = factories.recruiterUser.generateRecruiterUser();

    await use(recruiterUser);
  },
  candidateUser: async ({ factories }, use) => {
    const candidateUser = factories.candidateUser.generateCandidateUser();

    await use(candidateUser);
  },
